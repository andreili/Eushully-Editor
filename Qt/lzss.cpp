...
LZSS_PACK_DATA *create_lzss_pack_data(void)
{
    ...
    if ((dat = (LZSS_PACK_DATA*)malloc(sizeof(LZSS_PACK_DATA))) == NULL) {
        //printf("Failed to allocate memory for LZSS_PACK_DATA\n");
        return NULL;
    }
...
int lzss_write(unsigned char * outputbuf, int outputsize, LZSS_PACK_DATA *dat, int size, unsigned char *buf, int last)
{
    int outputindex = 0;
    int i = dat->i;
    int c = dat->c;
        ...
        if ((mask <<= 1) == 0) {                  /* shift mask left one bit */
            ...
            */
            ...
            for (i=0; i<code_buf_ptr; i++) {
                outputbuf[outputindex++] = dat->code_buf[i];
                //if (pack_ferror(file)) {
...
LZSS_UNPACK_DATA *create_lzss_unpack_data(void)
{
    ...
    if ((dat = (LZSS_UNPACK_DATA*)malloc(sizeof(LZSS_UNPACK_DATA))) == NULL) {
...
int lzss_read(QByteArray &inputbuf, int inputsize, LZSS_UNPACK_DATA *dat, int s, QByteArray &buf)
{
    int inputindex = 0;
    int i = dat->i;
    int j = dat->j;
...
pos1:
            ...
            //	break;
            i = inputbuf.at(inputindex++) & 0x000000ff;
            if (inputindex > inputsize) {
                break;
            }
            //if ((j = pack_getc(file)) == EOF)
            //	break;
            j = inputbuf.at(inputindex++) & 0x000000ff;
            if (inputindex > inputsize) {
                break;
            }
            i |= ((j & 0xF0) << 4);
            j = (j & 0x0F) + THRESHOLD;
            //printf("Repeated: i=%X, j=%X\n", i, j);
            for (k=0; k <= j; k++) {
...
int unlzss(QByteArray &inputbuf, int inputlen, QByteArray &outputbuf, int outputlen) {
...
{
    LzssSectorHeader hdr;
    fd.read((char*)&hdr, sizeof(LzssSectorHeader));

    int  len  = hdr.length;
    QByteArray buff = fd.read(len);

    out_len  = hdr.original_length;
    //out_buff.resize(out_len);

    if (len == out_len)
    {
        out_buff = buff;
        //memcpy(out_buff.data(), buff, out_len);
    } else
    {
